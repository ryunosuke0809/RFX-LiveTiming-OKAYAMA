...
flowchart LR
  ...
  Files["保存済ログ\n(smis_raw_*.txt)"] -->|TCP/XML 再生| VS["MOLA_Timing-VirtualServer\n(Windows / 開発機)"]
  VS -.->|開発時のみ| Receiver
...
| MOLA_Timing-Receiver | .NET 8 + WPF (C#) / SMIS TCP クライアント + XMLパーサー + ローカル WS サーバー |
| MOLA_Timing-VirtualServer | .NET 8 + WPF (C#) / 保存ログ再生 TCP サーバー |
...
- Windows 計時室向けの常駐アプリ（Receiver / VirtualServer）は信頼性・ネイティブ統合を重視し .NET 8 + WPF を採用。Socket.IO に依存せずネイティブ WebSocket で配信することで、C# と TypeScript の両側からシンプルに扱える。
...
**実装内容:**
- Node.js + Express + Socket.IO サーバー構築
- SQLite (better-sqlite3) でのデータ永続化
- REST API: セッション一覧 / リザルト取得 / CSVダウンロード / 個別ドライバーデータ
- WebSocket: リアルタイムStandings/Passing/Message配信
- フロントエンドのダミーデータ接続をWebSocket接続に切替

### Phase 3: MOLA_Timing-Receiver + MOLA_Timing-VirtualServer (Windows / .NET 8 + WPF)
...
- 保存: 生 XML (`smis_raw_YYYYMMDD.txt`) + 解析済 JSONL (`smis_parsed_YYYYMMDD.txt`) + SQLite ミラー
  ...
  server/                -- Phase 2 (Cloud, Node.js)
    src/
      ...
      db/
        ...
        archive.ts        -- 日付別DB管理・自動アーカイブ

  windows/               -- Phase 3 (.NET 8 + WPF)
    RfxTiming.sln
