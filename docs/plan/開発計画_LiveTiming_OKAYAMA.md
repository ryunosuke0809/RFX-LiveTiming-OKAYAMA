...
```mermaid
flowchart LR
  MOLA["MOLA計測システム\n(SMIS/TCP)"] -->|XML over TCP| Receiver["受信アプリ\n(岡山計時室)"]
...
| バックエンドAPI | Node.js + Express + Socket.IO |
| データベース | SQLite (better-sqlite3) -- 日付別ファイル運用 |
| SMIS受信アプリ | Node.js (TCP Socket クライアント + XMLパーサー) |
| デプロイ先 | VPS (nginx reverse proxy) |
| 状態管理 | React Context + useReducer |

**選定理由**: リアルタイム更新が必要なテーブルUIにはReactのVirtual DOMが最適。SMISがXML形式のため、Node.jsのXMLパーサーで統一的に処理できる。
...
### Phase 3: SMIS受信アプリ (岡山計時室設置)

- TCPソケットクライアント (MOLA SMIS接続)
- XMLパーサー (NULL終端で分割 → XML解析)
- マスターデータ / 計測データ / メッセージのJSON変換
- クラウドサーバーへのHTTP POST送信
- 接続断→自動再接続ロジック
...
RFX-LiveTiming-OKAYAMA/
  ...
  receiver/              -- Phase 3
    src/
      smis-client.ts      -- TCP接続
      xml-parser.ts       -- XMLパーサー
      sender.ts           -- クラウド送信
```
